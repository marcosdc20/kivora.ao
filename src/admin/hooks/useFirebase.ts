...
export function useCompanies() {
  ...
  const fetchFallback = useCallback(async () => {
    try {
      ...
      console.warn('Fallback de empresas Firestore:', err);
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsubscribe = onSnapshot(
        ...
        (err) => {
          console.warn('onSnapshot companies ordenado falhou, tentando fallback simples...', err);
          fetchFallback();
        }
      );
    } catch {
  ...
  return { companies, loading, error, addCompany, updateCompany, deleteCompany, refresh: fetchFallback };
}
...
export function useLicenses() {
  ...
  const fetchFallback = useCallback(async () => {
    try {
      ...
      console.warn('Fallback de licenças Firestore:', err);
      setError(err.message);
    } finally {
      setLoading(false);
    }
  ...
  return { licenses, loading, error, refresh: fetchFallback };
}
